...
erpnext.pos.PointOfSale = class Rem {};
erpnext.SerialNoBatchSelector  = class Rem {};

erpnext.show_serial_batch_selector = function(frm, d, callback, on_close, show_dialog) {
		console.log("Opened from insid");
	}
frappe.pages['point-of-sale'].on_page_load = function(wrapper) {
	...
	frappe.db.get_value('POS Settings', {name: 'POS Settings'}, 'is_online', (r) => {
		if (r && !cint(r.use_pos_in_offline_mode)) {
			// online
			wrapper.pos = new erpnext.pos.PointOfSale1(wrapper);
...
//~ erpnext.selling.SellingController._set_batch_number=function(doc) {
...
erpnext.pos.PointOfSale1 = class PointOfSale1 {
	constructor(wrapper) {
	...
	make_cart() {
		this.cart = new POSCart1({
			...
			events: {
				...
				on_coupon_code_change: (coupon_code) => {
					this.frm.set_value('pos_coupon_code', coupon_code);
				},
				...
				get_loyalty_details: () => {
					...
					if (this.frm.doc.customer) {
						frappe.call({
							method: "customizer.customizer.point_of_sale.get_loyalty_program_details_with_points",
							args: {
							...
							callback: function(r) {
								...
								else if (!r.message.loyalty_points) {
									var loyalty_details = {
										loyalty_points: 0,
										loyalty_program: '',
										expense_account: '',
	...
	show_serial_batch_selector = function(frm, d, callback, on_close, show_dialog) {
		frappe.require("assets/customizer/js/serial_no_batch_selector.js", function() {
			new erpnext.CustomSerialNoBatchSelector({
				frm: frm,
				item: d,
				warehouse_details: {
					type: "Warehouse",
	...
	select_batch_and_serial_no(row) {
		...
		this.show_serial_batch_selector(this.frm, row, () => {
	...
	on_close(item) {
		...
		console.log(this.cart.exists(item.item_code, item.batch_no));
	...
	update_cart_data(item) {
		...
		frappe.run_serially([
			...
			() => 		frappe.dom.unfreeze(),
			() => this.cart.update_discount_fields_from_item(),
		]);
	}
				...
				if(!r.exc) {
					...
					if (r.message) {
						...
						frappe.call({
							...
							args: {
								'doctype': 'POS Profile',
								'filters': {'name': this.frm.doc.pos_profile},
								'fieldname': [
									'auto_select_batch',
									"free_batch"
								]
							...
							callback: function(r) {
								if (!r.exc) {
									console.log("ASDFGHJKJHGFDSDASFDFHGJKJLKJGFHDGFHGJKJLKJKHJGHF");
									console.log(r);
							...
							this.frm.auto_select_batch = r.message.auto_select_batch;
							this.frm.free_batch = r.message.free_batch;
							});
...
class Payment1 {
	...
	get_fields() {
		...
		fields = fields.concat([
			...
			},
			{
				...
				default: me.frm.doc.write_off_amount,
				onchange: () => {
					me.update_cur_frm_value('sales_person', () => {
						frappe.flags.sales_person = false;
						me.update_sales_person();
					});
				}
			},
			{
	...
	set_flag() {
		...
		frappe.flags.sales_person = true;
	}
	...
	update_sales_person() {
		this.dialog.set_value("sales_person", this.frm.doc.sales_person);
	}
		...
		if (this.dialog.get_value("redeem_loyalty_points")) {
			frappe.run_serially([
				...
				frappe.run_serially([
					...
					() => this.dialog.set_value("loyalty_amount", 0),
					() => this.update_payment_amount(),
					() => this.show_paid_amount(),
				]);
			}
...
class POSItems1 {
	...
	make_fields() {
		...
		this.search_field.$input.on('input', (e) => {
			...
			this.last_search = setTimeout(() => {
				...
				this.filter_items({ search_term:search_term,  item_group: item_group,strict:0});
			}, 300);
		});
		//barcode 
		this.search_br_field = frappe.ui.form.make_control({
			df: {
				fieldtype: 'Data',
				label: __('barcode (Ctrl + b)'),
				placeholder: __('Search by item codeor barcode')
			},
			parent: this.wrapper.find('.search-field'),
			render_input: true,
		});
		...
		frappe.ui.keys.on('ctrl+b', () => {
			this.search_br_field.set_focus();
		});

		this.search_br_field.$input.on('input', (e) => {
			clearTimeout(this.last_search);
			this.last_search = setTimeout(() => {
				const search_term = e.target.value;
				const item_group = this.item_group_field ?
				...
				this.filter_items({ search_term:search_term,  item_group: item_group,strict:1});
			}, 300);
		});
	...
	filter_items({ search_term='', item_group=this.parent_item_group ,strict=0 }={}) {
		if (search_term) {
		...
		this.get_items({search_value: search_term, item_group ,strict})
	...
	reset_search_field() {
		...
		this.search_br_field.set_value('');
		this.search_br_field.$input.trigger("input");
	}
	...
	get_items({start = 0, page_length = 40, search_value='', item_group=this.parent_item_group ,strict =0}={}) {
		const price_list = this.frm.doc.selling_price_list;
		return new Promise(res => {
			frappe.call({
				method: "customizer.customizer.point_of_sale.get_items",
				freeze: true,
				args: {
					start,
					page_length,
...
class POSCart1 {
	...
	make() {
		this.make_dom();
		this.make_coupon_code_field_name();
		this.make_coupon_code_field();
	...
	reset() {
		...
		frappe.model.set_value(this.frm.doctype, this.frm.docname,'pos_coupon_code', "")
		this.coupon_code_field.set_value("");
	...
	make_coupon_code_field_name() {
		if (this.frm.allow_edit_discount){
			this.coupon_code_name_field= frappe.ui.form.make_control({
				df: {
					fieldtype: 'Data',
					label: 'Coupon Code',
					fieldname: 'coupon_code_name',
					options: 'Coupon Code',
					onchange: () => {
						if (this.coupon_code_name_field.get_value){					
							new Promise(res => {
								frappe.call({
									method: "customizer.customizer.point_of_sale.get_coupon_code_doc",
									freeze: true,
									args: {
										"coupon_code":this.coupon_code_name_field.get_value()
									}
								}).then(r => {
									if(r.message){
										this.coupon_code_field.set_value(r.message["coupon_code"]);
										frappe.model.set_value(this.frm.doctype, this.frm.docname,
											'pos_coupon_code', r.message["coupon_code"])
										...
										if (r.message["error"]){
											...
											frappe.model.set_value(this.frm.doctype, this.frm.docname,
												...
												.then(() => {
													...
													discount_wrapper.val(0);
													discount_wrapper.trigger('change');
													this.update_discount_fields_from_item();
												});
				...
				parent: this.wrapper.find('.coupon-code-field'),
				render_input: true
			});

			this.coupon_code_name_field.set_value(this.frm.doc.coupon_code_name);
		}
	}
	make_coupon_code_field() {
		if (this.frm.allow_edit_discount){
			this.coupon_code_field= frappe.ui.form.make_control({
				df: {
					fieldtype: 'Link',
					label: 'Coupon Name',
					...
					read_only: 1,
					onchange: () => {
						this.events.on_coupon_code_change(this.coupon_code_field.get_value());
						new Promise(res => {
							frappe.call({
								method: "customizer.customizer.point_of_sale.get_coupon_code",
								freeze: true,
								args: {
									"coupon_code":this.coupon_code_field.get_value()
								}
							}).then(r => {
								if(r.message){
									const discount_percentage = flt(r.message["discount_percentage"],
			...
			this.coupon_code_field.set_value(this.frm.doc.pos_coupon_code);
		}
	}
	...
	make_numpad() {
		...
		var pay_class = {}
		pay_class[__('Pay')]='btn btn-primary'
		this.numpad = new NumberPad({
			button_array: [
				[1, 2, 3, Qty],
				[4, 5, 6, Disc],
	...
	update_discount_fields_from_item() {
			new Promise(res => {
						frappe.call({
							method: "customizer.customizer.point_of_sale.get_coupon_code",
							freeze: true,
