...
class CustomPOSClosingEntry(StatusUpdater):
	...
	def on_submit(self):
		self.update_opening_entry(for_cancel=False)
		frappe.db.set_value("POS Closing Entry", self.name, "status", "Submitted")

	def on_cancel(self):
		self.update_opening_entry(for_cancel=True)
		frappe.db.set_value("POS Closing Entry", self.name, "status", "Cancelled")
