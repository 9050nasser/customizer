...
class MaterialRequestPlaning(StockController):
	def get_feed(self):
	...
	def validate_item(self):
		...
		for item in self.get("items"):
			...
			for f in ("uom", "stock_uom", "description","brand", "item_name", "expense_account",
	...
	def get_item_details(self, args=None, for_update=False):
		item = frappe.db.sql("""select i.name, i.stock_uom, i.description, i.image, i.brand,i.item_name, i.item_group,
...
@frappe.whitelist()
def get_uniuqe_items(from_warehouse,to_warehouse, posting_date, posting_time, company):
	lft, rgt = frappe.db.get_value("Warehouse", from_warehouse, ["lft", "rgt"])
	lft2, rgt2 = frappe.db.get_value("Warehouse", to_warehouse, ["lft", "rgt"])
	items = frappe.db.sql("""
		select i.name, i.item_name, bin.warehouse ,i.stock_uom,i.item_group, i.brand
		from tabBin bin, tabItem i
		where i.name=bin.item_code and i.disabled=0 and i.is_stock_item = 1
		and i.has_variants = 0 and i.has_serial_no = 0 and bin.actual_qty >0
		and exists(select name from `tabWarehouse` where lft >= %s and rgt <= %s and name=bin.warehouse)
		and i.name not in (select i1.name
		from tabBin bin1, tabItem i1
		where i1.name=bin1.item_code and i1.disabled=0 and i1.is_stock_item = 1
		and i1.has_variants = 0 and i1.has_serial_no = 0 and bin1.actual_qty >0
		and exists(select name from `tabWarehouse` where lft >= %s and rgt <= %s and name=bin1.warehouse)
		)
	""", (lft, rgt,lft2, rgt2))
