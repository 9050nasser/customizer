...
frappe.ui.form.on("Material Request Planing", {
	setup: function (frm) {
		...
		// formatter for Material Request Planing item
		...
		if(frm.doc.docstatus == 1) {
			...
			frm.add_custom_button(__('Create Material Request'), function() {
				frappe.model.with_doctype('Material Request', function() {
					...
					mr.set_from_warehouse = frm.doc.set_from_warehouse
					mr.set_from_warehouse = frm.doc.set_warehouse
					mr.schedule_date = frm.doc.schedule_date
					...
					items.forEach(function(item) {
						...
						mr_item.schedule_date = item.schedule_date;
						mr_item.warehouse = item.warehouse;
						mr_item.from_warehouse = item.from_warehouse;
	...
	get_unique_items_from_source: function(frm) {
		console.log("Hit1")
		frappe.call({
			method:"customizer.customizer.doctype.material_request_planing.material_request_planing.get_uniuqe_items",
			args: {
				from_warehouse: frm.doc.set_from_warehouse,
				to_warehouse: frm.doc.set_warehouse,
				posting_date: frm.doc.schedule_date,
			...
			callback: function(r) {
				...
				for(var i=0; i< r.message.length; i++) {
					...
					if(!d.schedule_date) d.schedule_date =frm.doc.schedule_date ;
					if(!d.from_warehouse) d.from_warehouse =frm.doc.set_from_warehouse ;
					if(!d.warehouse) d.warehouse =frm.doc.set_warehouse ;
					...
					callback: function (r) {
						if (!r.message) {
							...
							$.each(r.message, function (i, item) {
								var d = frappe.model.add_child(cur_frm.doc, "Material Request Planing Item", "items");
...
frappe.ui.form.on("Material Request Planing Item", {
	qty: function (frm, doctype, name) {
		const item = locals[doctype][name];
		if (flt(item.qty) < flt(item.min_order_qty)) {
			frappe.msgprint(__("Warning: Material Request Planinged Qty is less than Minimum Order Qty"));
		}
