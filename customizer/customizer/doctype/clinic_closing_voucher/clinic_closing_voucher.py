...
class ClinicClosingVoucher(Document):
	@frappe.whitelist()
	...
	def validate(self):
		self.validate_difference()
		self.validate_collected_amount()
		user = frappe.get_all('Clinic Closing Voucher',
			filters = {
				'user': self.user,
				'docstatus': 1
			},
		...
		if user:
			frappe.throw(_("Clinic Closing Voucher alreday exists for {0} between date {1} and {2}"
				.format(self.user, self.period_start_date, self.period_end_date)))

	def validate_difference(self):
		dif = 0 
		if self.get("payment_reconciliation"):
	...
	def validate_collected_amount(self):
		...
		self.total_amount = collected_amount
		...
		for invoice in invoice_list:
			if self.pos_profile:
				self.append('sales_invoices_summary', {
					...
					'qty_of_items': invoice['total_qty'],
	...
	def get_payment_reconciliation_details(self):
		...
		return frappe.render_template("customizer/customizer/doctype/clinic_closing_voucher/closing_voucher_details.html",
	...
	if invoice_list:
		...
		for d in inv_change_amount:
			for det in inv_mop_detail:
				if det["owner"] == d["owner"] and det["mode_of_payment"] == d["mode_of_payment"]:
...
def get_tax_details(invoice_list):
    ...
    for invoice in invoice_list:
        ...
        # Iterate over taxes in the Sales Invoice
        for t in doc.taxes:
            print(f"Processing tax: {t.account_head}, rate: {t.rate}, amount: {t.tax_amount}")
            tax_details[t.account_head][t.rate] += t.tax_amount
    
    # Construct the final tax_breakup list from the tax_details dictionary
    for account_head, rates in tax_details.items():
        for rate, amount in rates.items():
            tax_breakup.append({'account_head': account_head, 'rate': rate, 'amount': amount})
    
    print(f"Final tax_breakup: {tax_breakup}")
    return tax_breakup
...
def get_sales_summary(invoice_list):
	...
	total_qty = sum(item['total_qty'] for item in invoice_list)
...
def get_invoices(filters):
	return frappe.db.sql("""select a.name, a.base_grand_total as grand_total,
		a.base_net_total as net_total, a.total_qty
		...
		and a.name not in 
        (select b.invoice from `tabClinic Closing Voucher` aa
		join `tabPOS Closing Voucher Invoices` b
		on b.parent=aa.name
        where aa.posting_date >= %(from_date)s
        and aa.posting_date <= %(to_date)s 
        and aa.company=%(company)s
        and aa.pos_profile = %(pos_profile)s
        and aa.name != %(doc)s
        )
		""",
		filters, as_dict=1)
