...
class EmptySubStockReconciliationItemsError(frappe.ValidationError): pass

class SubStockReconciliation(StockController):
	def __init__(self, *args, **kwargs):
		super(SubStockReconciliation, self).__init__(*args, **kwargs)
	...
	def remove_items_with_no_change(self):
		...
		if not items:
			frappe.throw(_("None of the items have any change in quantity or value."),
				EmptySubStockReconciliationItemsError)
	...
	def validate_data(self):
		...
		for row_num, row in enumerate(self.items):
			...
			if key in item_warehouse_combinations:
				print(key)
				print("asdäaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
				self.validation_messages.append(_get_msg(row_num, _("Duplicate entry {0}".format(key))))
			else:
	...
	def get_gl_entries(self, warehouse_account=None):
		...
		return super(SubStockReconciliation, self).get_gl_entries(warehouse_account,
