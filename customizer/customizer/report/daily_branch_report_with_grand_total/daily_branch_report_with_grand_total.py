...
def get_conditions(filters):
    ...
    if filters.get("from_time"): 
        conditions += " and si.posting_time>='{0}' ".format(filters.get("from_time"))
    if filters.get("to_time"): 
        conditions += " and si.posting_time<='{0}' ".format(filters.get("to_time"))
    return conditions
...
def get_data(filters):
    ...
    for entry in sales_data:
        ...
        if cost_center not in cost_center_data:
            cost_center_data[cost_center] = {'cost_center': cost_center, 'grand_total': 0.0}
        
        cost_center_data[cost_center][posting_date] = grand_total
        cost_center_data[cost_center]['grand_total'] += grand_total  # Add to total grand amount
