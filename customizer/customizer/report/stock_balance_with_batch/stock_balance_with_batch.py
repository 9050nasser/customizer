...
def get_columns(filters):
	...
	columns = [
		...
		#{"label": _("Keey"), "fieldname": "key", "fieldtype": "Data", "options": "", "width": 90},
		{"label": _("Batch"), "fieldname": "batch_no", "fieldtype": "Link", "options": "Batch", "width": 90},
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql("""
		select
			...
			sle.item_code as name, sle.voucher_no,sle.batch_no
		from
...
def get_item_warehouse_map(filters, sle):
	...
	i = 1
	for d in sle:
		print(str((d.batch_no,d.qty_after_transaction ,d.actual_qty,d.voucher_no,d.stock_value_difference)))
		if not d.batch_no :
			d.batch_no = "NO-@VUCHER"
			i+= 1
		...
		if key not in iwb_map:
			iwb_map[key] = frappe._dict({
				...
				"val_rate": 0.0,"key":str(key),
			})

		qty_dict = iwb_map[(d.company, d.item_code, d.warehouse,d.batch_no)]

		if d.voucher_type == "Stock Reconciliation" and  d.batch_no == "NO-@VUCHER":
		...
		# ~ if d.voucher_type == "Stock Reconciliation":
			...
			# ~ print(str((d.batch_no)))

			# ~ print (qty_dict.bal_qty)
			# ~ print (d.voucher_no)
			# ~ print (d.qty_after_transaction)
			# ~ print (qty_dict.bal_qty)
			# ~ print ("aaaaaaaaaaaaaaaaaa")
			# ~ qty_diff = flt(d.qty_after_transaction) - qty_dict.bal_qty
		# ~ else:
		...
		if value_diff != 0 and flt(d.actual_qty) ==0:
			
			# ~ qty_diff = qty_diff + flt(d.stock_value_difference/d.valuation_rate)
			qty_diff = qty_diff + flt(d.quantity_difference)
			print("q  : "+str(qty_diff))
			# ~ print("q1 : "+str(qty_diff1))
		if d.valuation_rate == 0 :
			valuation_rate = 0
		else:
			print("q  : "+str(flt(d.stock_value_difference/d.valuation_rate)))
			# ~ print("q1 : "+str(flt(d.quantity_difference)))
