...
def execute(filters=None):
    ...
    for d  in data  :
        ...
        if d["mada"]  != "none":
            ...
            if d["mada"] == "less":
                d["commission_rate"] = 0.008
                d["commission"] = 0.008*d["collected_amount"]
                d["commission_tax"] = 0.15*d["commission"]
            if d["mada"] == "return":
        ...
        for pe in ("master","Master","visa","Visa", "Amircan Express") :
            if pe in str(d["mode_of_payment"]):
                d["commission_rate"] = 0.025
                d["commission"] = 0.025*d["collected_amount"]
                d["commission_tax"] = 0.15*d["commission"]
...
# ~ select c.mode_of_payment  , c.collected_amount from `tabPOS Closing Voucher` as p join `tabPOS Closing Voucher Details` as c on p.name =c.parent where p.name = "POS-CLO-2021-00421"
