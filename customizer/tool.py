...
def submit_all_attendance():
    """
    This script fetches all draft attendance records and submits them in bulk.
    """
    try:
        # Fetch all attendance records with status 'Draft'
        ...
        if not attendance_records:
            print("No draft attendance records found.")
            return

        # Iterate through each attendance record and submit
        for record in attendance_records:
            try:
                attendance_doc = frappe.get_doc("Attendance", record["name"])
                attendance_doc.submit()
                print(f"Attendance record {record['name']} submitted successfully.")
            except Exception as e:
                print(f"Failed to submit record {record['name']}: {e}")

        print("All attendance records have been processed.")
    except Exception as main_e:
        print(f"Error fetching attendance records: {main_e}")

# Execute the function
if __name__ == "__main__":
    submit_all_attendance()
...
def validate_max_discount(doc, method):
    if not doc.is_return:
        if doc.grand_total == 0:
            all_items_are_free = all(item.get("is_free_item") for item in doc.items)
            if all_items_are_free:
                return
            else:
                frappe.throw(_("Zero total invoice is only allowed if items are Free Items."))

        if doc.coupon_code:
            pricing_rule = frappe.db.get_value('Coupon Code', {'name': doc.coupon_code}, ['pricing_rule'], as_dict=True)
            if pricing_rule:
                prdoc = frappe.get_doc("Pricing Rule", pricing_rule["pricing_rule"])
                if not prdoc.get("items"):
                    return

                valid_coupon = False
                for item in doc.get("items"):
                    for rule_item in prdoc.get("items"):
                        if item.item_code == rule_item.item_code:
                            valid_coupon = True
                            break
                ...
                if doc.additional_discount_percentage != prdoc.discount_percentage:
                    frappe.throw(_("Discount Percentage (% {0}) does not match the Coupon (% {1})").format(
                        doc.additional_discount_percentage, prdoc.discount_percentage))

                doc.calculate_taxes_and_totals()

                expected_discount_amount = (doc.total * prdoc.discount_percentage) / 100.00
                if doc.discount_amount and (doc.discount_amount != expected_discount_amount):
                    frappe.throw(_("Discount Amount ({0}) does not match the expected Coupon Discount ({1})").format(
                        doc.discount_amount, expected_discount_amount))

        
        if not doc.coupon_code and doc.pos_profile:
            pos_profile = frappe.get_doc("POS Profile", doc.pos_profile)
            max_discount = pos_profile.max_discount

         
            if doc.additional_discount_percentage and doc.additional_discount_percentage > max_discount:
                frappe.throw(_("Discount is more than the limit allowed (% {0})").format(max_discount))

            
            for item in doc.get("items"):
                if item.discount_percentage > max_discount:
             
                    if not item.get("is_free_item") or item.discount_percentage != 100:
                        frappe.throw(_("Discount is more than the limit allowed (% {1}) for item {0}").format(
                            item.item_name, max_discount))

        
            max_discount_amount = (doc.total * max_discount) / 100.00
            if doc.discount_amount and doc.discount_amount > max_discount_amount:
                frappe.throw(_("Discount Amount ({0}) is more than the limit allowed ({1})").format(
                    doc.discount_amount, max_discount_amount))


def allow_zero_invoice(doc, method):
	if doc.grand_total == 0:
		if getattr(doc, "additional_discount_percentage", 0) == 100:
			return

		all_items_free = all(getattr(item, "is_free_item", 0) == 1 for item in doc.items)
		if all_items_free:
			return

		 
		all_items_discounted = all(item.get("rate", 0) == 0 for item in doc.items)
		if all_items_discounted:
			return

		
		frappe.throw(_("You cannot submit a zero-amount invoice unless all items are free or have 100% discount."))


def set_is_free_items(doc, method=None):
    # Get all active Free Items rules
    free_item_rules = frappe.get_all(
        'Free Items',
        filters={'enabled': 1},
        fields=['item_code']
    )

    if not free_item_rules:
        return

    free_item_codes = {rule.item_code for rule in free_item_rules}

    # Go through items in the invoice
    for item in doc.items:
        if item.item_code in free_item_codes:
            item.is_free_item = 1
        else:
            item.is_free_item = 0
...
def validate_sales_person_pos(doc, method):
    if doc.custom_sales_person and doc.is_pos and doc.custom_hit_sales_person == 0:
        sales_person_doc = frappe.get_doc("Sales Person",doc.custom_sales_person )
        child = frappe.new_doc("Sales Team")
        child.parent = doc.name
        child.parenttype = "POS Invoice"
        child.sales_person = doc.custom_sales_person
        child.allocated_percentage = 100.00
        child.commission_rate = flt(sales_person_doc.commission_rate,2)
        child.allocated_amount = flt(doc.base_net_total * child.allocated_percentage / 100.0,2)
        child.incentives = flt(child.allocated_amount * child.commission_rate / 100.0,2)
        doc.sales_team.append(child)
        doc.custom_hit_sales_person =1
...
def update_contract_end_dates():
    ...
    for employee in employees:
        ...
        contract_end_before_75_day = add_to_date(contract_end_date, days=-75)
        frappe.db.set_value("Employee", employee.name, "contract_end_before_75_day", contract_end_before_75_day)
...
#def add_contract_end_date(doc, method):	
#	if doc.date_of_joining:
#		joining_date = getdate(doc.date_of_joining)
#		current_year = getdate(today().year)
#		
#		update_joining_date = joining_date_replace(year=current_year)
		
		
#		doc.contract_end_date = add_years(doc.date_of_joining, 1 )
    

def clear_contract_end_fields(doc, method=None):
    if doc.status == "Left":
        doc.contract_end_date = None
        doc.contract_end_before_75_day = None
...
def update_custom_sales_person_for_invoices():
    # Define the date range
    start_date = "2024-07-01"

    # SQL Query to fetch Sales Invoices with no `custom_sales_person` and having a `sales_team`
    query = """
        SELECT DISTINCT si.name 
        FROM `tabSales Invoice` si
        INNER JOIN `tabSales Team` st ON st.parent = si.name
        WHERE si.custom_sales_person IS NULL
          AND si.docstatus = 1
          AND si.posting_date >= %s
    """
    invoices = frappe.db.sql(query, (start_date,), as_dict=True)

    print(f"Invoices without custom_sales_person but with sales_team: {len(invoices)}")
    print(invoices)

    for invoice in invoices:
        # Fetch the full Sales Invoice document
        print(f"Processing Invoice: {invoice['name']}")
        doc = frappe.get_doc("Sales Invoice", invoice["name"])
        
        # Check if the sales_team exists and get the first sales_person
        if doc.sales_team and doc.sales_team[0].sales_person:
            first_sales_person = doc.sales_team[0].sales_person
            
            # Update the `custom_sales_person` field in the database without updating the modified date
            frappe.db.set_value(
                "Sales Invoice", 
                invoice["name"], 
                "custom_sales_person", 
            ...
            # Log and print the update
            frappe.logger().info(
                f"Updated custom_sales_person to '{first_sales_person}' for Sales Invoice {invoice['name']}"
            )
            print(f"Updated custom_sales_person to '{first_sales_person}' for Sales Invoice {invoice['name']}")
...
# Function to parse time strings into `datetime.time` objects
def parse_time(value):
    if isinstance(value, str):
        return datetime.strptime(value, "%H:%M").time()
    return value  # Already a `datetime.time` object

# Function to calculate actual working hours and overtime
def calculate_working_and_overtime(doc):
    # Ensure the presence of required fields
    if not doc.custom_in_time or not doc.custom_out_time or not doc.custom_offical_working_hrs:
        #frappe.msgprint("Please ensure 'In Time', 'Out Time', and 'Official Working Hours' are set.")
        return

    # Parse `custom_in_time` and `custom_out_time`
    in_time = parse_time(doc.custom_in_time)
    out_time = parse_time(doc.custom_out_time)
    ...
    # Adjust for overnight shifts (if `out_time` is earlier than `in_time`)
    if out_datetime < in_datetime:
        out_datetime += timedelta(days=1)

    # Calculate actual working hours as `timedelta`
    actual_working_timedelta = out_datetime - in_datetime
    doc.custom_actual_working_hrs = (datetime.min + actual_working_timedelta).time()  # Store as `Time`

    # Parse official working hours (stored in HH:MM format)
    if isinstance(doc.custom_offical_working_hrs, str) and ":" in doc.custom_offical_working_hrs:
        hours, minutes = map(int, doc.custom_offical_working_hrs.split(":"))
        official_working_timedelta = timedelta(hours=hours, minutes=minutes)
    else:
        frappe.throw("Official Working Hours must be in HH:MM format.")

    # Calculate overtime as `timedelta`
    overtime_timedelta = max(actual_working_timedelta - official_working_timedelta, timedelta(0))
    doc.custom_overtime = (datetime.min + overtime_timedelta).time()  # Store as `Time`

    # Optional: Log debug information for verification
    # frappe.log_error(
    #     title="Debug: Time Calculation",
    #     message=f"""
    #     In Time: {doc.custom_in_time}, Out Time: {doc.custom_out_time}
    #     Actual Working Hrs: {doc.custom_actual_working_hrs}
    #     Official Working Hrs: {doc.custom_offical_working_hrs}
    #     Overtime: {doc.custom_overtime}
    #     """
    # )



def before_submit_update(doc, method):
    calculate_working_and_overtime(doc)

def on_submit_update(doc, method):
    calculate_working_and_overtime(doc)
...
@frappe.whitelist()
def calculate_leave_days(doc, method=None):
    from_date = getdate(doc.from_date) if doc.from_date else None
    to_date = getdate(doc.to_date) if doc.to_date else None
    half_day_date = getdate(doc.half_day_date) if doc.half_day_date else None

    if not from_date or not to_date:
        frappe.throw("From Date and To Date are required to calculate leave days.")
    ...
    if from_date.month == to_date.month and from_date.year == to_date.year:
        days_in_start_month = (to_date - from_date).days + 1
        days_in_end_month = 0
    else:

        days_in_start_month = days_in_month(from_date.year, from_date.month) - from_date.day + 1


        days_in_end_month = 0
        intermediate_date = from_date.replace(day=1)
        while intermediate_date < to_date.replace(day=1):
            intermediate_date = intermediate_date.replace(
                month=intermediate_date.month + 1 if intermediate_date.month < 12 else 1,
                year=intermediate_date.year + (1 if intermediate_date.month == 12 else 0),
                day=1,
            )
            if intermediate_date.month == to_date.month and intermediate_date.year == to_date.year:
                days_in_end_month += to_date.day  
            else:
                days_in_end_month += days_in_month(intermediate_date.year, intermediate_date.month)


    total_leave_days = days_in_start_month + days_in_end_month


    if half_day_date and from_date <= half_day_date <= to_date:
        total_leave_days -= 0.5
        if half_day_date.month == from_date.month and half_day_date.year == from_date.year:
            days_in_start_month -= 0.5
        elif half_day_date.month == to_date.month and half_day_date.year == to_date.year:
            days_in_end_month -= 0.5


    doc.custom_days_in_start_month = days_in_start_month
    doc.custom_days_in_end_month = days_in_end_month
    doc.total_leave_days = total_leave_days


    frappe.log_error(
        title="Leave Days Calculation",
        message=frappe.as_json({
            "From Date": from_date,
            "To Date": to_date,
            "Half Day Date": half_day_date,
            "Days in Start Month": days_in_start_month,
            "Days in End Month": days_in_end_month,
...
@frappe.whitelist()
def calculate_rate_per_hrs(doc, method=None):
    # Ensure custom_total_salary exists
    if not hasattr(doc, 'custom_total_salary') or not doc.custom_total_salary:
        frappe.throw("Custom Total Salary is required to calculate the rate per hour.")

    # Convert custom_total_salary to a float
    try:
        custom_total_salary = flt(doc.custom_total_salary)
    except ValueError:
        frappe.throw("Custom Total Salary must be a numeric value.")

    # Default working hours per day
    total_working_hours_per_day = 8

    # Calculate custom rate per hour
    custom_rate_per_hrs = (custom_total_salary / 30) 

    # Update the custom_rate_per_hrs field in the document
    if hasattr(doc, 'custom_rate_per_hrs'):
        doc.custom_rate_per_hrs = custom_rate_per_hrs
    else:
        frappe.throw("Custom Rate Per Hours field is not defined in the document.")

    # Log the calculation for debugging
    frappe.log_error(
        title="Custom Rate Per Hours Calculation",
        message={
            "Custom Total Salary": custom_total_salary,
            "Total Working Hours per Day": total_working_hours_per_day,
            "Custom Rate Per Hours": custom_rate_per_hrs,
        }
    )
...
@frappe.whitelist()
def calculate_custom_amount(doc, method=None):
    # Ensure required fields exist
    if not hasattr(doc, 'custom_rate_per_hrs') or not doc.custom_rate_per_hrs:
        frappe.throw("Custom Rate Per Hours is required.")
    if not hasattr(doc, 'total_leave_days') or not doc.total_leave_days:
        frappe.throw("Total Leave Days is required.")

    # Convert fields to numeric types
    custom_rate_per_hrs = flt(doc.custom_rate_per_hrs)
    total_leave_days = flt(doc.total_leave_days)

    # Calculate custom total amount
    custom_total_amount = custom_rate_per_hrs * total_leave_days

    # Update custom_total_amount field in the document
    if hasattr(doc, 'custom_total_amount'):
        doc.custom_total_amount = custom_total_amount
    else:
        frappe.throw("Custom Total Amount field is not defined in the document.")

    # Log the calculation for debugging
    frappe.log_error(
        title="Custom Total Amount Calculation",
        message={
            "Custom Rate Per Hours": custom_rate_per_hrs,
            "Total Leave Days": total_leave_days,
            "Custom Total Amount": custom_total_amount,
        }
    )
...
def on_submit(doc, method):
    ...
    create_monthly_additional_salaries(
        employee=doc.employee,
        from_date=doc.from_date,
        to_date=doc.to_date,
        custom_total_amount=doc.custom_total_amount,
        salary_component="leave (خصم )"
    )

def create_monthly_additional_salaries(employee, from_date, to_date, custom_total_amount, salary_component):
    from_date = getdate(from_date)
    to_date = getdate(to_date)
    custom_total_amount = float(custom_total_amount)  
    ...
    while current_date <= to_date:
        ...
        prorated_amount = (custom_total_amount / total_days) * days_in_period
        ...
        additional_salary_link = frappe.utils.get_link_to_form("Additional Salary", additional_salary.name)
        frappe.msgprint(
            msg=f"Additional Salary created successfully: {additional_salary_link}",
            title="Success",
            indicator="green"
        )
...
def update_leave_allocation():
    leave_allocations = frappe.get_all(
        "Leave Allocation",
        filters={"docstatus": 1, "leave_type": "سنوية"},
        fields=["name", "custom_employee_type", "new_leaves_allocated", "total_leaves_allocated", "custom_leave_allocation_changes", "employee"]
    )
    
    for allocation in leave_allocations:
        employee_status = frappe.get_value("Employee", allocation.employee, "status")

        if employee_status == "Left":
            continue  # Skip if employee has left

        increments = {"Employee": 0.08333333, "Labor": 0.05833333}
        increment = increments.get(allocation.custom_employee_type, 0)

        if increment > 0:
            # Store current values before update
            current_new_leaves = allocation.new_leaves_allocated or 0
            current_total_leaves = allocation.total_leaves_allocated or 0
            
            # Calculate new values
            new_leaves = current_new_leaves + increment
            total_leaves = new_leaves  # Assuming both should have the same value

            # Track changes
            change_details = []
            if current_new_leaves != new_leaves:
                change_details.append(f"New Leaves Allocated from {current_new_leaves} to {new_leaves}")
            if current_total_leaves != total_leaves:
                change_details.append(f"Total Leaves Allocated from {current_total_leaves} to {total_leaves}")

            # Apply updates to Leave Allocation
            frappe.db.set_value("Leave Allocation", allocation.name, {
            ...
            # Log change history
            if change_details:
                existing_changes = allocation.custom_leave_allocation_changes or ""
                new_change_entry = f"Administrator changed the following: " + ", ".join(change_details)
                updated_changes = (existing_changes + "\n" + new_change_entry).strip()
                frappe.db.set_value("Leave Allocation", allocation.name, "custom_leave_allocation_changes", updated_changes)

def set_cost_price(doc, method):
    for item in doc.items:
        if item.item_code and item.warehouse:
            valuation_rate = frappe.db.get_value("Stock Ledger Entry", 
                {"item_code": item.item_code, "warehouse": item.warehouse}, "valuation_rate")
            item.cost_price = valuation_rate or 0.0
...
def update_employee_type():
    print("Running daily employee_type updater...")

    # Fetch employees with employee_type 'Labor' and status 'Active'
    employees = frappe.get_all(
        ...
        fields=["name", "employee_type", "date_of_joining", "status"]
    )

    if not employees:
        print("No employees found with employee_type 'Labor' and status 'Active'.")
        return

    for emp in employees:
        if not emp.date_of_joining:
            print(f"Employee {emp.name} has no date of joining.")
            continue

        doj = emp.date_of_joining
        today = date.today()

        years_worked = today.year - doj.year
        if (today.month, today.day) < (doj.month, doj.day):
            years_worked -= 1

        print(f"{emp.name} - Date of Joining: {doj} - Years Worked: {years_worked} - Status: {emp.status}")

        if years_worked >= 5:
            frappe.db.set_value("Employee", emp.name, "employee_type", "Employee")
            print(f"Updated {emp.name} to 'Employee'")
        else:
            print(f"Skipped {emp.name}, not reached 5 years yet (only {years_worked})")

    frappe.db.commit()
    print("Employee type update task completed successfully.")
