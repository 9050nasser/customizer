    ...
    def get_salary(self, employee):
        ...
        if salary_structure:
            result = frappe.db.sql("""
                ...
                AND abbr IN ('TeA', 'B', 'SA', 'TA', 'IA', 'OA', 'MA', 'HA', 'M', 'GOSI', 'ot_1', 'p', 'lo', 'Bo', 'OT')
            """, salary_structure)
            ...
            result2 = frappe.db.sql("""
                ...
                FROM `tabSalary Detail` 
                WHERE parent=%s AND parentfield='earnings' 
                AND salary_component IN ('Basic', 'Housing Allowance')
            """, salary_structure)


            total_earnings = flt(result[0][0]) if result and result[0][0] is not None else 0
            basic_and_housing = flt(result2[0][0]) if result2 and result2[0][0] is not None else 0

            return cstr(total_earnings), round(total_earnings / 30, 2), round(basic_and_housing / 30, 2)
        else:
